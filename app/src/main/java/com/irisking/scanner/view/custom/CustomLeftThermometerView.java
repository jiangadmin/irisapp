...
public class CustomLeftThermometerView extends View {
...
//    private int mScaleMercuryLength;
    ...
    private int thermometerDefaultWidth = 12;
    private int thermometerDefaultHeight = 860;

    float optWidth;
    float optHeight;

    public CustomLeftThermometerView(Context context) {
        this(context, null);
    }

    public CustomLeftThermometerView(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//        mScaleMercuryLength = mTypedArray.getInt(R.styleable.ThermometerView_mercuryLength, 60);
        ...
        mercuryMargin = mTypedArray.getInt(R.styleable.ThermometerView_mercuryMargin, 3);
    ...
    void drawScaleDegree(Canvas canvas) {
        ...
        for (int i = 0; i < mDegreeArr.length; i++) {
            ...
            mDegreePaint.setTextSize(16 * optWidth);
            if (unitWidth == 0) {
                unitWidth = (int) mDegreePaint.measureText(mDegreeUnit);
                x = mThermometerWidth + unitWidth;
            }
            canvas.drawText(mDegreeUnit, x, y, mDegreePaint);

            mDegreePaint.setTextSize(20 * optWidth);
